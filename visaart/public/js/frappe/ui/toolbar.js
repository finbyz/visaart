...
import '../../../../../../frappe/frappe/public/js/frappe/form/linked_with';
...
frappe.ui.form.Toolbar = Class.extend({
	...
	set_title: function() {
		...
		if(this.frm.meta.search_fields){
            if(this.frm.doc.__islocal){
                this.page.set_search_area(''); 
            }
            else{
                var search_fields = this.frm.meta.search_fields;
                var searchvalues = ""
                var me = this.frm.doc
                var searchArr = search_fields.split(',');
                var lenght = searchArr.length
                searchArr.forEach(function(entry){
                    if (!--lenght){
                        searchvalues += me[entry] || ""
                    }
                    else{
                        searchvalues += me[entry]+" | "  || ""
                    }
                })
                this.page.set_search_area(searchvalues); 
            }
